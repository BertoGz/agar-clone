...
function checkForOrbCollisions(pData, pConfig, orbs, settings) {
  return new Promise((resolve, reject) => {
    ...
    orbs.forEach((orb, i) => {
      ...
      if (
        pData.posX + pData.orbRadius + orb.orbRadius > orb.posX &&
        pData.posX < orb.posX + pData.orbRadius + orb.orbRadius &&
        pData.posY + pData.orbRadius + orb.orbRadius > orb.posY &&
        pData.posY < orb.posY + pData.orbRadius + orb.orbRadius
      ) {
        // Pythagoras test(circle)
        distance = Math.sqrt(
          (pData.posX - orb.posX) * (pData.posX - orb.posX) +
            (pData.posY - orb.posY) * (pData.posY - orb.posY)
        );
        if (distance < pData.orbRadius + orb.orbRadius) {
          //COLLISION!!!
          pData.score += 1;
  ...
  return new Promise((resolve, reject) => {
    ...
    players.forEach((curPlayer, i) => {
      if (curPlayer.uid != playerId) {
        ...
        let pPosx = curPlayer.posX;
        let pPosy = curPlayer.posY;
        let pR = curPlayer.radius;
        // AABB Test - Axis-aligned bounding boxes
        if (
          pData.posX + pData.orbRadius + pR > pPosx &&
          pData.posX < pPosx + pData.orbRadius + pR &&
          pData.posY + pData.radius + pR > pPosy &&
          pData.posY < pPosy + pData.radius + pR
        ) {
          // Pythagoras test
          distance = Math.sqrt(
            (pData.posX - pPosx) * (pData.PosX - pPosx) +
              (pData.posY - pPosy) * (pData.PosY - pPosy)
          );
          if (distance < pData.orbRadius + pR) {
            //COLLISION!!
            if (pData.orbRadius > pR) {
              // ENEMY DEATH
